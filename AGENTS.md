...
RALPH_AGENT_CMD=./plans/adapters/Codex.sh ./plans/ralph.sh plans/sprints/YYYY-MM-DD-{slug} 25

# Single iteration:
RALPH_AGENT_CMD=./plans/adapters/Codex.sh ./plans/ralph_once.sh plans/sprints/YYYY-MM-DD-{slug}
```
...
cp /path/to/barrelbook/plans/adapters/Codex.sh plans/adapters/

chmod +x plans/ralph.sh plans/ralph_once.sh plans/run_agent.sh plans/adapters/Codex.sh

# Then paste the "Ralph Sprint Workflow" section above into your AGENTS.md
```